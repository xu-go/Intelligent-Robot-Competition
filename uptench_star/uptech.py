...
##初始化
    ...
    def FAN_Set_Speed(self,speed):              #风扇？
        if self.hPi >= 0:
            if speed>100:
                speed=100
            elif speed<0:
...
##初始化mpu6500
 ...
 ##获取加速度
    def MPU6500_GetAccel(self):
        so_up.mpu6500_Get_Accel(self.__MPU_DATA1)
        for i in range(3):
            self.ACCEL_DATA1[i] = self.__MPU_DATA1[i]
        return self.ACCEL_DATA1
##获取角速度
    def MPU6500_GetGyro(self):
        so_up.mpu6500_Get_Gyro(self.__MPU_DATA1)
        for i in range(3):
            self.GYRO_DATA1[i] = self.__MPU_DATA1[i]
        return self.GYRO_DATA1
##获取姿态数据，pitch roll yaw
    def MPU6500_GetAttitude(self):
        so_up.mpu6500_Get_Attitude(self.__MPU_DATA1)
        for i in range(3):
            self.ATTITUDE1[i] = self.__MPU_DATA1[i]
